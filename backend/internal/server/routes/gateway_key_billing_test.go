...
func TestGatewayRoutesKeyBillingInfoPathIsRegistered(t *testing.T) {
	...
	for _, route := range router.Routes() {
		if route.Method == http.MethodGet && route.Path == "/v1/kimai/billing" {
			return
		}
	}

	t.Fatal("GET /v1/kimai/billing should be registered")
}
...
func TestGatewayRoutesKeyBillingInfoEndToEnd(t *testing.T) {
	t.Run("missing credentials", func(t *testing.T) {
		...
		router.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/v1/kimai/billing", nil))
	...
	t.Run("standard mode", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodGet, "/v1/kimai/billing", nil)
		...
		require.Equal(t, "kimai.key_billing", body["object"])
