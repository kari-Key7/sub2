...
func cleanJSONSchemaRecursive(value any) any {
	...
	if props, ok := schemaMap["properties"].(map[string]any); ok {
		for pk, v := range props {
			cleaned := cleanJSONSchemaRecursive(v)
	...
	if items, ok := schemaMap["items"]; ok {
		...
		if itemsArr, ok := items.([]any); ok {
			...
			cleaned := cleanJSONSchemaRecursive(itemsMap)
			schemaMap["items"] = cleaned
		}
	}
	...
	if looksLikeSchema {
		...
		// 7.1 [SAFETY] 处理 Array 类型：Gemini 要求所有 type=="array" 必须有合法的 items Schema
			...
			if !hasItems || items == nil {
				...
				if len(itemsMap) == 0 {
					...
					if _, hasType := itemsMap["type"]; !hasType {
						if hasKey(itemsMap, "properties") {
							itemsMap["type"] = "object"
						} else if hasKey(itemsMap, "items") {
							itemsMap["type"] = "array"
						} else {
							itemsMap["type"] = "string"
						}
					}
					// 递归校验 inner items（如果 items 也是 array）
					if itemType, _ := itemsMap["type"].(string); itemType == "array" {
						if innerItems, hasInner := itemsMap["items"]; !hasInner || innerItems == nil {
