...
vi.mock("vue-i18n", async () => {
  ...
  const translations: Record<string, string> = {
    ...
    "admin.settings.upstreamBillingProbe.description": "定期获取 OpenAI API Key 所连接上游 KimAI 站点声明的计费倍率。",
...
describe("admin SettingsView payment visible method controls", () => {
  ...
  it("does not render external payment documentation links", async () => {
    ...
    // 私有部署：支付页不再链接到公开仓库文档，避免暴露上游项目来源
    const externalLinks = wrapper
      .findAll("a")
      .filter((node) => (node.attributes("href") || "").includes("github.com/") && (node.attributes("href") || "").includes("/docs/"));
