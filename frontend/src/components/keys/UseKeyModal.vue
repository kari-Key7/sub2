...
export XAI_API_KEY="${apiKey}"`
  ...
  // Shape follows Grok Build user guide (~/.grok/docs + custom-models) and production-ready KimAI setups.
  ...
  const configContent = `# Grok Build CLI → KimAI Grok group (API key auth).
# Docs: ~/.grok/docs/user-guide/05-configuration.md + 11-custom-models.md
# Verify after save: grok inspect
#
# IMPORTANT: api_backend must be "responses" for KimAI Grok (POST /v1/responses).
...
# Prefer API key when using a custom gateway (matches KimAI).
...
description = "Grok 4.5 via KimAI (Responses)"
...
# Imagine tools: model IDs go to KimAI media endpoints (not the text [model.*] catalog).
...
function generateGrokCodexFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  switch (shell) {
    case 'cmd':
      envPath = 'Command Prompt'
      envContent = `set KIMAI_API_KEY=${apiKey}`
      break
    case 'powershell':
    case 'windows':
      envPath = 'PowerShell'
      envContent = `$env:KIMAI_API_KEY="${apiKey}"`
      break
    default:
      envPath = 'Terminal'
      envContent = `export KIMAI_API_KEY="${apiKey}"`
  }

  const configContent = `# Codex CLI → KimAI Grok group
...
# Grok/KimAI path is HTTP/SSE; disable WS (Codex may otherwise try WebSocket first)
...
function generateRoutedCodexFiles(
  ...
  const envContent = isWindows
    ? `$env:KIMAI_API_KEY="${apiKey}"`
    : `export KIMAI_API_KEY="${apiKey}"`

  const configContent = `# Codex CLI -> KimAI ${label} group
model_provider = "kimai"
model = "${model}"
review_model = "${model}"
disable_response_storage = true
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  if (platform === 'gemini') {
    ...
    // Custom provider pointing at KimAI OpenAI-compatible Responses/Chat endpoints.
    provider[platform].npm = '@ai-sdk/openai-compatible'
    provider[platform].name = 'Grok via KimAI'
